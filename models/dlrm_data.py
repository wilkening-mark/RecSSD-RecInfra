    ...
    def create_sequential_forward_ops(self, id_qs = None, len_qs = None, fc_q = None):
        ...
        if self.args.sls_type == 'dram':
            ...
            for i in range(len(self.ln_emb)):
                ...
                self.libFlashRec.unvme_write_table(FlatTableType(*W.flatten().tolist()),
                                                   c_int(m), c_int(n), i)
...
def slsload(sls_type, lru, libFlashRec, m_spa, X=None, S_lengths=None, S_indices=None, qid=0):
    ## sparse features
    listResult = None
    if sls_type == 'base':
        for i in xrange(len(S_lengths)):
            batchsize = len(S_lengths[i])
            embed_per_result = S_lengths[i][0]
        ...
        for i in range(len(S_lengths)):
            ...
            split = [S_indices[i]]
            if batchsize > 128:
                split = np.split(np.array(S_indices[i]), (batchsize/128))
                batchsize = 128
            listResult = []
            for indices in split:
                ...
                result = libFlashRec.unvme_sparse_length_sum(
                        IndexType(*flatInd),
                        c_int(m_spa), c_int(batchsize), c_int(embed_per_result), c_int(i), c_int(qid))
    ...
    if args.data_generation == "dataset":
        ...
        if args.data_generation == "random":
            ...
            with open(args.data_file, 'w') as f:
                pickle.dump([nbatches, lX, lS_l, lS_i], f)
            print("Exiting")
            sys.exit()
        else:
