...
from caffe2.python import brew, core, dyndep, model_helper, net_drawer, workspace, rnn_cell
...
class DIEN_Wrapper(object):
    ...
    def run_queues(self, ids, lengths, fc, batch_size):
        ...
        workspace.FeedBlob( self.seq_input_blob, sequence_lengths)
        workspace.FeedBlob( self.hid_input_blob, initial_h_data)

        workspace.RunNetOnce( self.seq_input_net.Proto() )
        workspace.RunNetOnce( self.hid_input_net.Proto() )

        for i in range(num_tables):
           workspace.FeedBlob( self.id_input_blobs[i], ids[i])
           workspace.RunNetOnce( self.id_input_nets[i].Proto() )

           workspace.FeedBlob( self.len_input_blobs[i], lengths[i])
...
class DIEN_Net(object):
    def create_mlp(self, ln, model, tag, fc_q = None):
        ...
        for i in range(1, ln.size):
            ...
            workspace.FeedBlob(tag_fc_w, W)
            workspace.FeedBlob(tag_fc_b, b)
    ...
    def create_input(self, X, S_lengths, S_indices, T):
        ...
        workspace.FeedBlob( 'seq_lengths', sequence_lengths )

        initial_h_data = np.zeros( (X.shape[0], self.args.hidden_size) ).astype(np.float32)
        workspace.FeedBlob( 'initial_h', initial_h_data)
...
if __name__ == "__main__":
    ...
    for  k in range(args.nepochs):
      for j in range(nbatches):
        ...
        dien.run(lX[j], lS_l[j], lS_i[j]) # args.enable_profiling
